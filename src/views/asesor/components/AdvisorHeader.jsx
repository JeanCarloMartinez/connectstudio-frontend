  ...
  return (
    <header className="bg-white shadow-sm p-4 flex items-center justify-between lg:justify-end border-b border-gray-200">
      ...
      <div className="flex items-center space-x-4 relative">
        <span className="text-gray-700 font-medium hidden sm:block">Bienvenido, Asesor</span>
        {/* Icono de notificaciones */}
        <button
          className="relative focus:outline-none"
          onClick={handleNotificationClick}
        >
          <svg className="w-7 h-7 text-indigo-600 hover:text-indigo-800" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        {showDropdown && (
          ...
          >
            <div className="flex items-center justify-between p-4 border-b">
              <span className="text-indigo-700 font-semibold">Notificaciones</span>
              <button
            ...
            <ul className="max-h-60 overflow-y-auto">
              {notificaciones.length === 0 ? (
                <li className="p-4 text-gray-500">No tienes notificaciones nuevas.</li>
              ) : (
                notificaciones.map((n) => (
                  <li key={n.id} className="p-4 border-b last:border-b-0 text-gray-700 hover:bg-indigo-50 transition">
                    {n.mensaje}
                  </li>
                ))
              )}
